...
const LoadingForCheck = () => {
  ...
  console.log("Payment Data: ", paymentData);  // Log the payment data to verify it's correct
  
  // Function to handle the payment verification
  const verifyPayment = async () => {
    if (!paymentData) {
      setErrorMessage('معرّف الدفع غير متوفر.');  // Payment ID not available
      setLoading(false);
      return;
    }
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_MAIN_URL}tickets/checkout/${paymentData}`,
        {},
        {
          headers: {
            'Content-Type': 'application/json',
            // You can include any other headers like Authorization if needed
          },
          withCredentials: true, // Ensure this is needed, especially for sessions/cookies
        }
      );
      ...
      if (response.status === 201) {
        setSuccess(true);
        // Navigate to "Thank You" page after a successful payment
        setTimeout(() => {
...
export default LoadingForCheck;
